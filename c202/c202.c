...
void Stack_Init( Stack *stack )
{
	if (stack == NULL)
	{
		// cant initialize pointer to null
		Stack_Error(SERR_INIT);
	}
	else
	{
		// initialize stack index to -1 = stack is empty
		stack->topIndex = -1;
	}
}
...
{
	// stack is full when topIndex is STACK_SIZE - 1 because we are indexing from 0
...
void Stack_Top( const Stack *stack, char *dataPtr ) 
{
	...
	{
		// if stack is empty there is no top if stack
		Stack_Error(SERR_TOP);
	}
	else
	{
		// assign data from stack array on topIndex position to dataPtr
...
void Stack_Pop( Stack *stack )
{
	if (!Stack_IsEmpty(stack))
	{
		// if stack is not empty we can pop value by decrementing topIndex (we dont need to erase value from stack itself because there is no fuction to access it)
		stack->topIndex--;
	}
}
...
void Stack_Push( Stack *stack, char data ) 
{
	...
	{
		// if stack is full we cant add any other values to the top
		Stack_Error(SERR_PUSH);
	}
	else
	{
		// increment topIndex and add new value to it
		stack->topIndex++;
